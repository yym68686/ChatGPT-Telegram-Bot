...
async def start(update, context): # 当用户输入/start时，返回文本
    ...
    run_async(update.message.reply_text(message, parse_mode='MarkdownV2'))

async def error(update, context):
    logger.warning('Update "%s" caused error "%s"', update, context.error)
    run_async(
        context.bot.send_message(chat_id=update.message.chat_id, text="出错啦！请重试。", parse_mode='MarkdownV2')
    )
